...
sample_data(mh_TE_PSV_transformed.ps)$Mh_level
...
upsetda_edit <- get_upset(mh_TE_PSV_transformed.ps, factorNames="Mh_level") ## ASV
...
taxa_to_keep <- taxa_names(mh_TE_PSV_transformed.ps) %in% taxa_names

# Use prune_taxa with the logical vector to subset the phyloseq object
mh_TE_PSV_filtered <- prune_taxa(taxa_to_keep, mh_TE_PSV_transformed.ps)
...
abundance_threshold <- median(taxa_sums(mh_TE_PSV_transformed.ps))
#abundance_threshold <- 1000


# Identify low and high abundance taxa
low_abundance_taxa <- taxa_sums(mh_TE_PSV_transformed.ps) <= abundance_threshold
high_abundance_taxa <- taxa_sums(mh_TE_PSV_transformed.ps) > abundance_threshold

# Create two new phyloseq objects
mh_TE_PSV_low_abundance <- prune_taxa(low_abundance_taxa, mh_TE_PSV_transformed.ps)
mh_TE_PSV_high_abundance <- prune_taxa(high_abundance_taxa, mh_TE_PSV_transformed.ps)
