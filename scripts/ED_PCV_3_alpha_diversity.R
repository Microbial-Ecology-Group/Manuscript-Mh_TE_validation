...
plot1 <- ggplot(sample_data(mh_TE_PSV_transformed.ps), mapping=aes(x= Mh_level , y = Observed, fill = Concentration , color = Concentration )) +
  theme_bw() + 
...
mhpp_pw_obs_Concentration <- pairwise.wilcox.test(sample_data(mh_TE_PSV_transformed.ps)$Observed, sample_data(mh_TE_PSV_transformed.ps)$Concentration, p.adjust.method = "BH")
mhpp_pw_obs_Concentration$p.value # p-values for Shannons
...
# Plot richness and diversity
# Richness by sample type
plot2 <-ggplot(sample_data(mh_TE_PSV_transformed.ps), mapping=aes(x= Mh_level , y = Shannon, fill = Concentration , color = Concentration )) +
  theme_bw() + 
...
mhpp_pw_shann_Concentration <- pairwise.wilcox.test(sample_data(mh_TE_PSV_transformed.ps)$Shannon, sample_data(mh_TE_PSV_transformed.ps)$Concentration, p.adjust.method = "BH")
mhpp_pw_shann_Concentration$p.value # p-values for Shannons
